...
const Header: React.FC<HeaderProps> = ({ user }) => {
  ...
  const navigation = [
    ...
    { name: 'Histórico', href: '/history', current: location.pathname === '/history' },
          ...
          <div className="flex items-center space-x-4">
            {user && (
              <>
                ...
                <button onClick={logout} className="px-4 py-2 bg-gray-800 text-white rounded-lg hover:bg-gray-700 transition-colors">Sair</button>
              </>
            )}
          </div>
        </div>
