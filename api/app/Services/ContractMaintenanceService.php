...
class ContractMaintenanceService
{
    ...
    {
        Log::info("🔄 [ContractMaintenanceService] Iniciando renovação automática de contrato", [
        ...
        Log::info("✅ [ContractMaintenanceService] Contrato renovado com sucesso", [
    ...
    {
        Log::info("💰 [ContractMaintenanceService] Processando cobrança recorrente", [
            'contract_id' => $contractId
        ]);
        ...
        if ($isCycleStart) {
            ...
            Log::info("📊 [ContractMaintenanceService] Cobrança proporcional aplicada", [
                'contract_id' => $contractId,
        ...
        if ($valorAPagar > 0) {
            ...
            Log::info("💳 [ContractMaintenanceService] Pagamento recorrente registrado", [
    ...
    {
        Log::info("🔧 [ContractMaintenanceService] Iniciando manutenção diária");
        ...
        try {
            ...
            Log::info("📋 [ContractMaintenanceService] Contratos expirados encontrados", [
                'count' => $expiredContracts->count()
            ]);

            foreach ($expiredContracts as $contract) {
                try {
                    $this->renew($contract->id);
                    $results['contracts_renewed']++;
                } catch (\Exception $e) {
                    $results['errors'][] = "Erro renovando contrato {$contract->id}: " . $e->getMessage();
                    Log::error("❌ [ContractMaintenanceService] Erro ao renovar contrato", [
            ...
            Log::info("💳 [ContractMaintenanceService] Contratos com cobrança pendente encontrados", [
                'count' => $contractsDue->count()
            ]);

            foreach ($contractsDue as $contract) {
                try {
                    $this->processRecurring($contract->id);
                    $results['recurring_payments_processed']++;
                } catch (\Exception $e) {
                    $results['errors'][] = "Erro processando recorrência contrato {$contract->id}: " . $e->getMessage();
                    Log::error("❌ [ContractMaintenanceService] Erro ao processar cobrança recorrente", [
                        'contract_id' => $contract->id,
                        'error' => $e->getMessage()
                    ]);
                }
            ...
            Log::error("💥 [ContractMaintenanceService] Erro geral na manutenção diária", [
                'error' => $e->getMessage()
            ]);
        }

        Log::info("✅ [ContractMaintenanceService] Manutenção diária concluída", $results);
        return $results;
    }
    ...
    {
        return \App\Models\UserBalance::getTotalBalanceForUser($userId);
    }

    /**
     * Aplicar saldo em um pagamento
     */
    private function applyBalanceToPayment(int $userId, float $paymentAmount): array
    {
        Log::info('💰 [ContractMaintenanceService] Aplicando saldo no pagamento', [
            'user_id' => $userId,
        ...
        Log::info('💰 [ContractMaintenanceService] Saldo calculado', [
            'user_id' => $userId,
    ...
    {
        $credits = \App\Models\UserBalance::forUser($userId)
        ...
        foreach ($credits as $credit) {
            ...
            \App\Models\UserBalanceTransaction::create([
                'user_id' => $userId,
